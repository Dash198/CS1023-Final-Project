...
\begin{itemize}
    ...
    \end{itemize}
    \item Alternatively, ensure that the \texttt{raw\_source} folder and \texttt{MIA.java} are in the same directory. Compile the source code inside the folder, and then compile \texttt{MIA.java}, and run it as shown:\\\texttt{java MIA <int/float> <add/sub/mul/div> <op1> <op2>}\\Or you can run \texttt{infarith.py} which handls both the compilation and execution.
\end{itemize}
...
\begin{itemize}
    ...
    \item The approach used is very basic and primitive without much optimization, so any big operations (like multiplication of numbers with 20,000 digits) can be very slow, but they are accurate.
\end{itemize}
